...
fn faq_blocks() -> Vec<Block<'static>> {
    vec![
        ...
        Block::Body("Yes — in Settings → Bibliography, use the folder-browse button to choose a Kartoteka vault directory instead of a file. Entries load live: add or edit something in Kartoteka and it appears in the citation popup, sidebar, and reference manager within a second or two, with no restart. The reference manager's rename/add-entry/export actions stay .bib-only, since the vault is edited in Kartoteka, not Zerkalo."),
        Block::Gap,
        ...
        Block::Body("Every `Ctrl+S` saves a timestamped copy of the current file to `~/.local/share/zerkalo/snapshots/<project>/<file>/`. The last 50 snapshots per file are kept. Open ☰ → Version History → Saved Versions… to see the timeline, compare with the current text, and restore any version."),
        Block::Gap,
