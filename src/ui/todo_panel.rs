...
// Other lines (headers, notes the user typed by hand) are preserved verbatim
// as `passthrough` — never shown as checkboxes, never touched by toggling or
// deleting items — and re-emitted verbatim above the checkbox items on save,
// so hand-edited content in the file survives round-tripping through the UI.
...
fn parse_items(s: &str) -> (Vec<Item>, Vec<String>) {
    let mut items = Vec::new();
    let mut passthrough = Vec::new();
    for l in s.lines() {
        if let Some(rest) = l.strip_prefix("- [ ] ") {
            items.push(Item { text: rest.to_string(), done: false });
        } else if let Some(rest) = l.strip_prefix("- [x] ") {
            items.push(Item { text: rest.to_string(), done: true });
        } else if l.trim().is_empty() || l.starts_with("---") {
            // Blank lines and the "--- Completed ---" separator are
            // regenerated by items_to_string(); don't duplicate them.
            continue;
        } else {
            passthrough.push(l.to_string());
        }
    }
    (items, passthrough)
}

fn items_to_string(passthrough: &[String], items: &[Item]) -> String {
...
struct TodoList {
    ...
    passthrough: Rc<RefCell<Vec<String>>>,
...
impl TodoList {
    fn new() -> Self {
        ...
        let passthrough: Rc<RefCell<Vec<String>>> = Rc::new(RefCell::new(Vec::new()));
        let save_path: Rc<RefCell<Option<PathBuf>>> = Rc::new(RefCell::new(None));
        let is_loading: Rc<RefCell<bool>> = Rc::new(RefCell::new(false));

        let list = Self { widget, list_box, items, passthrough, save_path, is_loading };
    ...
    fn load(&self, path: &PathBuf) {
        ...
        *self.is_loading.borrow_mut() = true;
        ...
        let (items, passthrough) = parse_items(&text);
        *self.items.borrow_mut() = items;
        *self.passthrough.borrow_mut() = passthrough;
        self.rebuild();
    ...
    fn schedule_save(&self) {
        ...
        let passthrough = self.passthrough.borrow().clone();
        let path_opt = self.save_path.borrow().clone();
        let text = items_to_string(&passthrough, &items);
...
mod tests {
    ...
    #[test]
    fn parse_items_splits_checkboxes_from_passthrough_text() {
        let (items, passthrough) = parse_items("## This Week\n- [ ] Draft outline\n- [x] Buy coffee\n");
        assert_eq!(passthrough, vec!["## This Week".to_string()]);
        assert_eq!(items.len(), 2);
        assert_eq!(items[0].text, "Draft outline");
        assert!(!items[0].done);
        assert_eq!(items[1].text, "Buy coffee");
        assert!(items[1].done);
    }

    #[test]
    fn passthrough_text_survives_a_save_round_trip() {
        // Reproduces the corruption bug: hand-editing a header into
        // global-todo.md, then toggling one item, used to permanently rewrite
        // the header as a bogus "- [ ] ## This Week" checkbox item.
        let original = "## This Week\n- [ ] Draft outline\n";
        let (mut items, passthrough) = parse_items(original);
        items[0].done = true; // simulate toggling the one real item
        let saved = items_to_string(&passthrough, &items);
        assert!(saved.starts_with("## This Week\n"), "passthrough line must survive verbatim: {saved:?}");
        assert!(!saved.contains("- [ ] ## This Week"), "header must not be coerced into a checkbox item");
    }

    #[test]
    fn parse_items_drops_blank_lines_and_completed_separator_without_treating_them_as_passthrough() {
        let (items, passthrough) = parse_items("- [ ] a\n\n--- Completed ---\n- [x] b\n");
