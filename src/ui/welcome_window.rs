    ...
    pub fn new(parent: &impl IsA<gtk4::Window>, is_first_run: bool) -> Self {
        ...
        if is_first_run {
            ...
            for item in [
                ...
                "New: the inline suggestion previews what will actually be inserted, and the status line leads with the function's signature",
                "New: completion remembers what you pick for a prefix (per project) and favours names already used in the document",
                "New: citations (@) and CV entries (!) behave like # — inline suggestion, status-line description, list only after two characters",
                "New: Escape now means \"not for this word\" — it stays quiet until you move on, and no longer deletes what you typed",
                "New: the hint says when only built-in snippets are available (tinymist not running), and the list says when it's showing a slice of the matches",
