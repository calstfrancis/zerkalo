...
- **The spell-check suggestions popover opening and closing instantly on right-click, again.** The 0.26.3 fix deferred `popup()` to the next idle cycle, reasoning that the paired button-release would finish dispatching first — true on X11, not reliably true on Wayland, where a fast right-click's press and release can land in the same main-loop iteration before any idle source runs at all, so the fix won zero time against the race it targeted. Replaced with a real 40ms delay, which can't lose that race regardless of how the backend batches input events — but that then exposed a second, previously-invisible bug: the poll loop that fills in hunspell's suggestions decided "the user already dismissed this" by checking `popover.is_visible()`, which is also false during the gap between this handler returning and the now-delayed popup() actually showing it. A poll tick landing in that gap read "not visible yet" as "already dismissed" and canceled itself permanently, before the popover had even appeared — so hunspell's answer arrived to an empty room and "Checking…" never updated. Dismissal is now tracked with an explicit flag set only in the popover's real close handler, not inferred from visibility timing.
...
- **Settings dialog cleaned up for plain language.** "Debounce" is now
  ...
  moved to a tooltip); the "Extras" tab — which actually held Bibliography,
  CV Elements, and Spell Check — is now "References & Spelling". The
  Autocorrect toggle used to appear twice (hamburger menu and Settings,
  wired to the same setting); Settings' copy is gone, the hamburger's quick
  toggle is the one control now, matching how the GOST font toggle already
  worked.
