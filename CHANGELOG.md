...
## [0.19.0-dev6] — Inline autocomplete and CV profiles, plus security, data-integrity, and correctness fixes

### Changed
- **Controls moved to where they belong.** The status bar now runs the full width of the window, under the sidebar as well as the editor — it reports on the document, not on one pane. The completion hint has the whole left half of it to itself, with every standing control packed to the far right, so a suggestion can show its name, what it does and its keys without being squeezed. Simple Mode and Focus moved up to the header beside Library; the compile-mode toggle (auto / on save / manual) moved up beside the compile buttons it describes; the root-file controls moved up beside the document title; and the GOST Type B font switch — a once-in-a-while setting, not a status — moved into the ≡ menu.
- **Project controls can be put away.** A single-file document has no root to choose, so the controls and the "main.typ detected" banner were just taking up space with no way to close them. The controls now carry a dismiss button that shuts them for that project and keeps the banner from coming back — remembered in the project's own config. The "project" toggle stays in the header, so one click brings them back.
...
- **Tab shows its work.** The inline suggestion now previews what will actually be inserted, not just the rest of the name: `#fig` shows `ure(image(""), caption: [Caption text])` dim ahead of the cursor, so accepting a snippet is no longer a leap of faith. The status line leads with the signature — `figure(image(""), caption: […])` — and falls back to prose only when there isn't one.
- **Completion learns from you.** The name you pick for a prefix is remembered per project and offered first next time, and names already used in the document outrank ones that aren't — so `#col` means something different in a file that already calls `#columns`. The list footer says when it's showing a slice ("8 of 137"), which is the cue to type another letter rather than scroll.
- **Citations work like functions now.** `@` and `!` got the same treatment as `#`: an inline ghost completing the key, the status line naming the entry and describing it, the list held back until two characters are typed. A bare `@` used to drop the whole bibliography over the text.
- **Escape means "not for this word".** It dismissed the suggestion and then the very next keystroke brought it back. Now it stays quiet until you move on to another word — and, crucially, it no longer deletes what you typed: Escape with the list open used to erase back to the `#`.
- **Zerkalo says when completions are limited.** With no language server running, `#` offers eleven built-in snippets and nothing said so. The hint now adds "built-in snippets only (tinymist not running)".
- **Suggestions say what they do and which key takes them.** The status bar names the current suggestion and describes it (`pagebreak — Force content to start on a new page · Tab insert · ↑↓ select · Esc dismiss`) while the ghost is showing, so the explanation never covers the text you're writing; a bare `#` prompts with what to do next instead. The list keeps a one-line key reminder at its foot.
- **Snippets are found by their Typst name, and by fragments of it.** They used to be matched on their display title, so `#pagebreak` — the thing you'd actually type — matched nothing, while the ghost text offered `Page break` and then inserted something else. Names are now the Typst identifier, and `#break` still finds `pagebreak`.
- **Suggestions no longer appear for reasons you can't see.** Matching briefly read item descriptions too, which is how typing `#column` offered `dropcap` (its description contains "decorative"). Matching is now on the name alone, ranked the way VS Code ranks: a name that starts with what you typed, then one where it starts a word, then anywhere in the name, then a loose letters-in-order match (only for three characters or more). The matched letters are emboldened in each row, and the best match is always the one selected.
- **The suggestion list fits its contents.** Rows were wrapping their descriptions over three lines, so two entries filled the box and the rest hid behind a scrollbar. Each row is now a single line — name, description, kind — ellipsized rather than wrapped, in a wider box that shows eight of them.
...
### Fixed
- **Pasting threw the editor to the top of the document.** After a paste, GTK animates the viewport to the very top — an eased curve over a dozen frames, with focus never leaving the editor and the cursor still mid-document, so nothing in Zerkalo asked for it and no signal declines it. Earlier attempts fixed the app's own bookkeeping and made no difference, because the movement wasn't the bookkeeping's doing. The viewport is now *held* for a moment after a paste: each frame of that animation is put straight back, so nothing visibly moves. A paste that lands off-screen still scrolls to where the text went.
- **The right-click menu covered the spelling suggestions.** The spell popover opened on right-click, and GtkTextView's own context menu opened on top of it, hiding the suggestions the right-click was for. Zerkalo now claims that click when it has suggestions to show, so only one menu appears.
