...
- **Snippets are found by their Typst name, and by fragments of it.** They used to be matched on their display title, so `#pagebreak` — the thing you'd actually type — matched nothing, while the ghost text offered `Page break` and then inserted something else. Names are now the Typst identifier, and the list matches anywhere in a name or its description, so `#break` finds `pagebreak` and `#quote` finds the block snippet.
